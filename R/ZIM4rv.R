...
#' Fisher's method (Fisher‐p)
#'
#' This function combines p-values using Fisher's method
#' for the omnibus test of testing the joint genetic effect.
#'
#' @param x a numeric vector containing p-values
#' @return a combined p-value indicating the joint effect
...
fisherp<-function(x){
  s=-2*log(x[1])-2*log(x[2])
  n=2*2
  p=pchisq(s,n,lower.tail = F)
  return(p)
}
...
#' @returns a list of 10 items including the name of gene, the number of rare variants in the gene
...
zimfrv <- function(phenodata, genedata, name = NULL, weights = "Equal", missing_cutoff = 0.15, max_maf = 1, model = "zip"){
  ...
  p_burden_fisher <- fisherp(c(p_fi_burden,p_lambda_burden))
  p_kernel_cauchy <- cauchyp(c(p_fi_kernel,p_lambda_kernel))
  p_kernel_fisher <- fisherp(c(p_fi_kernel,p_lambda_kernel))

  output <- data.frame(GeneName=name,No.Var=n_rare,
                       p_phi_burden=p_fi_burden,p_lambda_burden=p_lambda_burden,
                       p_phi_kernel=p_fi_kernel,p_lambda_kernel=p_lambda_kernel,
                       p_burden_cauchy=p_burden_cauchy,p_burden_fisher=p_burden_fisher,
                       p_kernel_cauchy=p_kernel_cauchy,p_kernel_fisher=p_kernel_fisher)
    ...
    p_burden_fisher <- fisherp(c(p_phi_burden,p_mu_burden))
    p_kernel_cauchy <- cauchyp(c(p_phi_kernel,p_mu_kernel))
    p_kernel_fisher <- fisherp(c(p_phi_kernel,p_mu_kernel))
    ...
    p_burden_fisher <- fisherp(c(p_pi_burden,p_mu_burden))
    p_kernel_cauchy <- cauchyp(c(p_pi_kernel,p_mu_kernel))
    p_kernel_fisher <- fisherp(c(p_pi_kernel,p_mu_kernel))
